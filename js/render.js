...
function rgbaString(rgb, alpha = 1) {
  ...
  return `rgba(${Math.round(rgb.r ?? 0)}, ${Math.round(rgb.g ?? 0)}, ${Math.round(rgb.b ?? 0)}, ${a})`;
}
...
function drawParticles(particles, params, dt) {
  ...
  const zoomRaw = Number.isFinite(params.zoom) ? params.zoom : 1;
  const zoom = clamp(zoomRaw, 0.5, 20);
  const halfWorldWidth = Math.max(state.world.width * 0.5, 1e-3);
  const halfWorldHeight = Math.max(state.world.height * 0.5, 1e-3);
  const widthScale = (state.logicalWidth * 0.5) / halfWorldWidth;
  const heightScale = (state.logicalHeight * 0.5) / halfWorldHeight;
  const scaleBase = Math.max(widthScale, heightScale);
  const scale = scaleBase * (zoom >= 1 ? zoom : 1);
...
function getArrayLengthLike(source) {
  if (!source || typeof source.length !== 'number') {
    return 0;
  }
  const length = Math.floor(source.length);
  return Number.isFinite(length) && length > 0 ? length : 0;
}

function ensureModelHudCapacity(kind, required) {
  ...
  const nextLength = Math.max(required, currentLevels.length > 0 ? currentLevels.length * 2 : 8);
  const nextLevels = new Float32Array(nextLength);
  if (currentLevels.length > 0) {
    nextLevels.set(currentLevels.subarray(0, hud[visibleKey]));
  }
  hud[levelKey] = nextLevels;

  const currentPolarity = hud[polarityKey];
  const nextPolarity = new Int8Array(nextLength);
  if (currentPolarity.length > 0) {
    nextPolarity.set(currentPolarity.subarray(0, hud[visibleKey]));
  }
  hud[polarityKey] = nextPolarity;
}

function computeNormalizedLevel(value) {
  if (!Number.isFinite(value)) {
    return 0;
  }
  const magnitude = Math.abs(value);
  const normalized = 1 - Math.exp(-magnitude * MODEL_LEVEL_RESPONSE);
...
function updateHudGroup(kind, source, limit) {
  ...
  if (count > 0) {
    for (let i = 0; i < count; i += 1) {
      const rawValue = Number(source[i]);
      const target = computeNormalizedLevel(rawValue);
      const prev = levels[i];
      const next = Number.isFinite(prev)
        ? prev + (target - prev) * MODEL_LEVEL_SMOOTHING
        : target;
      levels[i] = clamp(Number.isFinite(next) ? next : target, 0, 1);
      if (!Number.isFinite(rawValue) || rawValue === 0) {
        polarity[i] = 0;
      } else {
        polarity[i] = rawValue > 0 ? 1 : -1;
      }
    }
  }
  hud[visibleKey] = Math.max(hud[visibleKey], count);
  return count;
}
...
function finalizeHudGroup(kind, activeCount) {
  ...
  for (let i = activeCount; i < visible; i += 1) {
    const current = levels[i];
    const decayed = Number.isFinite(current)
      ? current + (0 - current) * MODEL_LEVEL_DECAY
      : 0;
    if (decayed <= MODEL_LEVEL_THRESHOLD) {
      levels[i] = 0;
      polarity[i] = 0;
    } else {
      levels[i] = clamp(decayed, 0, 1);
      polarity[i] = 0;
    }
  }
  let trimmed = visible;
  while (trimmed > activeCount && trimmed > 0 && levels[trimmed - 1] <= MODEL_LEVEL_THRESHOLD) {
    trimmed -= 1;
  }
...
function hasActiveLevels(levels, count) {
  if (!levels || count <= 0) {
    return false;
  }
  for (let i = 0; i < count; i += 1) {
    if (levels[i] > MODEL_LEVEL_THRESHOLD) {
      return true;
    }
  }
  return false;
...
function drawVerticalBars({
  ...
  const gap = count > 1 ? Math.min(6, (width / count) * 0.28) : 0;
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    if (level <= MODEL_LEVEL_THRESHOLD) {
      continue;
    }
    const barHeight = height * level;
    const bx = x + i * (barWidth + gap);
    ctx.fillStyle = rgbaString(colors.barFillRgb, 0.36 + level * 0.52);
    ctx.fillRect(bx, baseY - barHeight, barWidth, barHeight);
    const tipHeight = Math.max(2, Math.min(barHeight, 6));
    const tipY = Math.max(y, baseY - barHeight - tipHeight * 0.5);
    ctx.fillStyle = rgbaString(colors.tipFillRgb, 0.9);
    ctx.fillRect(bx, tipY, barWidth, Math.max(2, tipHeight));
  }
  ctx.restore();
...
function drawHorizontalBars({
  ...
  const gap = count > 1 ? Math.min(10, (height / count) * 0.3) : 0;
  const barHeight = Math.max(6, (height - gap * (count - 1)) / count);

  ctx.save();
  ctx.fillStyle = colors.barTrack;
  for (let i = 0; i < count; i += 1) {
    const by = y + i * (barHeight + gap);
    ctx.fillRect(x, by, width, barHeight);
  }
  ctx.restore();
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const by = y + i * (barHeight + gap);
    ctx.fillStyle = rgbaString(colors.barFillRgb, 0.4 + level * 0.5);
    ctx.fillRect(x, by, barWidth, barHeight);
    const tipWidth = Math.max(2, Math.min(barWidth, 8));
    if (tipWidth > 0.5) {
      ctx.fillStyle = rgbaString(colors.tipFillRgb, 0.94);
      ctx.fillRect(x + barWidth - tipWidth, by, tipWidth, barHeight);
    }
  }
  ctx.restore();
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    ctx.fillText(`${percent}%`, valueX, centerY);
  }
  ctx.restore();
}
...
function drawHudPanel({
  ...
  const radius = Math.min(16, Math.min(width, height) * 0.18);
  ctx.save();
  ...
  const padding = Math.max(12, Math.round(height * 0.12));
  const labelFontSize = Math.max(12, Math.round(height * 0.14));
  const valueFontSize = Math.max(10, Math.round(height * 0.1));

  ctx.save();
  ctx.font = `600 ${labelFontSize}px 'Rajdhani', 'Segoe UI', sans-serif`;
  ctx.textAlign = 'left';
  ctx.textBaseline = 'top';
  ctx.fillStyle = colors.label;
  ctx.fillText(label, x + padding, y + padding * 0.75);
  ctx.restore();

  const headerHeight = padding * 0.75 + labelFontSize + padding * 0.45;
  const contentTop = y + headerHeight;
  const contentLeft = x + padding;
  const contentWidth = Math.max(10, width - padding * 2);
  const contentHeight = Math.max(10, y + height - padding - contentTop);
  const lines = Math.max(2, Math.min(gridLines, orientation === 'vertical'
    ? Math.floor(contentHeight / 26)
    : Math.floor(contentWidth / 70)));

  ctx.save();
  ctx.strokeStyle = colors.grid;
  ctx.lineWidth = Math.max(1, colors.lineWidth * 0.65);
  if (orientation === 'vertical') {
    for (let i = 1; i < lines; i += 1) {
      const gy = contentTop + (contentHeight * i) / lines;
      ctx.beginPath();
      ctx.moveTo(contentLeft, gy);
      ctx.lineTo(contentLeft + contentWidth, gy);
      ctx.stroke();
    }
  } else {
    for (let i = 1; i < lines; i += 1) {
      const gx = contentLeft + (contentWidth * i) / lines;
      ctx.beginPath();
      ctx.moveTo(gx, contentTop);
      ctx.lineTo(gx, contentTop + contentHeight);
      ctx.stroke();
    }
  }
  ctx.restore();

  const valueFont = `500 ${Math.round(valueFontSize)}px 'JetBrains Mono', 'Roboto Mono', monospace`;
  if (orientation === 'vertical') {
    const labelBand = Math.min(contentHeight * 0.28, valueFontSize + padding * 0.6);
    const barHeight = Math.max(12, contentHeight - labelBand);
    const labelY = contentTop + barHeight + Math.max(2, (labelBand - valueFontSize) * 0.5);
    drawVerticalBars({
      ctx,
      x: contentLeft,
      y: contentTop,
    ...
    const valueColumn = Math.max(56, contentWidth * 0.22);
    const gaugeWidth = Math.max(24, contentWidth - labelColumn - valueColumn);
    const gaugeX = contentLeft + labelColumn;
    const labelX = contentLeft + labelColumn * 0.5;
    const valueX = gaugeX + gaugeWidth + valueColumn * 0.3;
    drawHorizontalBars({
      ctx,
      x: gaugeX,
      y: contentTop,
...
function drawModelHud() {
  ...
  const width = state.logicalWidth || 1;
  const height = state.logicalHeight || 1;
  ...
  const glowRgb = mixRgb(accentRgb, { r: 255, g: 255, b: 255 }, 0.18);
  const tipRgb = mixRgb(glowRgb, { r: 255, g: 255, b: 255 }, 0.35);
  const shadowRgb = mixRgb(accentRgb, state.palette.backgroundRgb, 0.55);
  const gridRgb = mixRgb(accentRgb, state.palette.backgroundRgb, 0.72);
  const labelRgb = mixRgb(accentRgb, { r: 255, g: 255, b: 255 }, 0.35);
  const hudColors = {
    background: rgbaString(shadowRgb, 0.22),
    border: rgbaString(glowRgb, 0.72),
    grid: rgbaString(gridRgb, 0.24),
    label: rgbaString(labelRgb, 0.92),
    barTrack: rgbaString(gridRgb, 0.18),
    barFillRgb: glowRgb,
    tipFillRgb: tipRgb,
    value: rgbaString(labelRgb, 0.86),
    valueDim: rgbaString(gridRgb, 0.78),
    lineWidth: Math.max(1, 1.2 / (state.pixelRatio * state.dynamicScale)),
  };

  if (hasFeatures) {
    const featuresWidth = Math.max(200, width * 0.3);
    drawHudPanel({
      ctx,
      x: margin,
      y: baseY,
      width: featuresWidth,
      height: panelHeight,
      label: 'INPUT VECTOR LEVELS',
      levels: state.modelHud.featureLevels,
      count: featuresVisible,
      colors: hudColors,
